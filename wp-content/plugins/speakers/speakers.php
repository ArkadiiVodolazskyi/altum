...
function speakers_styles() {
	wp_register_style( 'speakers_styles', plugins_url( 'speakers.css', __FILE__ ), rand(1,9999) );
	wp_enqueue_style( 'speakers_styles', 9999999 );
}

function speakers_styles_admin() {
	wp_register_style( 'speakers_styles', plugins_url( 'speakers_admin.css', __FILE__ ), rand(1,9999) );
	wp_enqueue_style( 'speakers_styles', 9999999 );
}
...
function speakers_templates( $template ) {
	if ( get_post_type() === 'speakers' ) {
		if (is_single()) {
			$template = __DIR__ . './single-speakers.php';
		} else if (is_archive()) {
			$template = __DIR__ . './archive-speakers.php';
		}
	}
	return $template;
}
